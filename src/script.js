...
i['checkPasswordsForUnsavedChanges'] = () => {
	let checkerTimeout;
	let saveTimeout;
...
k.addShortcut(
	{
		keys: 'ctrl + scroll',
		title: 'Increase or decrease size',
		group: 'Scroll',
		code: false
	}
);
...
document.addEventListener('wheel', function(event) {
    if (event.ctrlKey && k.tab == 'passwords') {

		if (event.shiftKey) {
			const targetElement = document.querySelector('.tab.passwords');
			if (targetElement) {
				let scale = parseFloat(getComputedStyle(targetElement).getPropertyValue('transform').split(',')[0].slice(7)) || 1;
        ...
        let fontSize = parseFloat(getComputedStyle(root).getPropertyValue('--sheet-font-size'));
		let pad = parseFloat(getComputedStyle(root).getPropertyValue('--sheet-pad'));
		let height = parseFloat(getComputedStyle(root).getPropertyValue('--sheet-height'));
		let maxWidth = parseFloat(getComputedStyle(root).getPropertyValue('--sheet-max-width'));
        ...
        if (event.deltaY < 0) {
			...
			height += 2.3;
			maxWidth += 3;
        } else if (event.deltaY > 0 && fontSize > 5) {
            fontSize -= 1;
			pad -= 0.5;
			height -= 2.3;
			maxWidth -= 3;
		...
		root.style.setProperty('--sheet-pad', `${pad}px`);
		root.style.setProperty('--sheet-height', `${height}px`);
		root.style.setProperty('--sheet-max-width', `${maxWidth}px`);
    }
});
